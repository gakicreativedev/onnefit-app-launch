...
import {
  PlayBold, StopBold, ClockCircleBold, CheckCircleBold, TrashBinTrashBold, ShareBold,
  RunningBold, WalkingBold, RunningRoundBold, StopwatchBold, WaterdropsBold, DumbbellBold, CupStarBold,
} from "solar-icon-set";
import { format } from "date-fns";
import { ptBR } from "date-fns/locale";

const ACTIVITY_ICONS: Record<string, typeof RunningBold> = {
  esteira: RunningBold,
  caminhada: WalkingBold,
  corrida: RunningRoundBold,
  bicicleta: RunningBold,
  bike_ergométrica: RunningBold,
  elíptico: StopwatchBold,
  corda: DumbbellBold,
  natação: WaterdropsBold,
  remo: DumbbellBold,
  escada: RunningBold,
};

const ACTIVITIES = [
  { key: "esteira", label: "Esteira", met: 8.0 },
  { key: "caminhada", label: "Caminhada", met: 3.5 },
  { key: "corrida", label: "Corrida", met: 9.8 },
  { key: "bicicleta", label: "Bicicleta", met: 7.5 },
  { key: "bike_ergométrica", label: "Bike Ergométrica", met: 6.8 },
  { key: "elíptico", label: "Elíptico", met: 5.0 },
  { key: "corda", label: "Corda", met: 11.0 },
  { key: "natação", label: "Natação", met: 8.0 },
  { key: "remo", label: "Remo", met: 7.0 },
  { key: "escada", label: "Escada", met: 9.0 },
];
...
export default function CardioTracker() {
  ...
  useEffect(() => {
    if (elapsed > 0) {
      saveCardioCache({ activity: selectedActivity, elapsed, distance, startedAt: Date.now() });
    }
  ...
  const ActivityIcon = ACTIVITY_ICONS[selectedActivity] || RunningBold;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
        {ACTIVITIES.map(act => {
          const Icon = ACTIVITY_ICONS[act.key] || RunningBold;
          return (
            <button
              key={act.key}
      ...
      <div className="rounded-[20px] bg-card p-6 flex flex-col items-center gap-4">
        <div className="flex items-center gap-2 text-muted-foreground text-sm font-medium">
          ...
          <ActivityIcon size={16} color="currentColor" />
          <span>{activityObj?.label}</span>
        </div>
        ...
        <div className="flex gap-3 w-full max-w-xs">
          {!running ? (
            <Button className="flex-1 rounded-2xl py-6 text-base font-bold" onClick={handleStart} disabled={saving}>
              <PlayBold size={20} color="currentColor" className="mr-2" />
              {elapsed > 0 ? "Continuar" : "Iniciar"}
            </Button>
          ) : (
            <Button variant="outline" className="flex-1 rounded-2xl py-6 text-base font-bold border-destructive text-destructive hover:bg-destructive/10 hover:text-destructive" onClick={handleStop}>
          ...
          {elapsed > 0 && !running && (
            <Button className="flex-1 rounded-2xl py-6 text-base font-bold" onClick={handleSave} disabled={saving}>
        ...
        <div className="space-y-2">
          ...
          <AnimatePresence>
            {sessions.map(session => {
              ...
              const Icon = ACTIVITY_ICONS[session.activity_type] || RunningBold;
              return (
                <motion.div
                  key={session.id}
        ...
        <DialogContent className="sm:max-w-sm rounded-[24px] p-0 overflow-hidden">
          <div ref={resultRef} className="bg-gradient-to-br from-primary via-primary to-primary/80 p-8 flex flex-col items-center gap-6 text-primary-foreground">
            <DialogHeader className="items-center">
              <DialogTitle className="text-lg font-black text-primary-foreground flex items-center gap-2">
                Cardio Concluído! <CupStarBold size={20} color="currentColor" />
              </DialogTitle>
            </DialogHeader>

            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-primary-foreground/20">
              {(() => { const RIcon = ACTIVITY_ICONS[resultDialog?.activityKey || ""] || RunningBold; return <RIcon size={40} color="currentColor" />; })()}
            </div>
