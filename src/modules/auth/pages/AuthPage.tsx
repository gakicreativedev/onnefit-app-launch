...
function getPasswordStrength(password: string, t: (key: string) => string) {
  ...
  if (score <= 1) return { score: 1, label: t("auth.passwordWeak"), color: "bg-destructive" };
  if (score <= 3) return { score: 2, label: t("auth.passwordMedium"), color: "bg-yellow-500" };
...
export default function AuthPage() {
  ...
  const MOTIVATIONAL = [
    t("auth.motivational1"),
    t("auth.motivational2"),
    t("auth.motivational3"),
    t("auth.motivational4"),
  ];
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    else { toast.success(t("auth.recoveryEmailSent")); setIsForgotPassword(false); }
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (!trimmedEmail || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(trimmedEmail)) { toast.error(t("auth.invalidEmail")); setLoading(false); return; }
        if (password.length < 6) { toast.error(t("auth.shortPassword")); setLoading(false); return; }
        const { data, error } = await supabase.auth.signUp({ email: trimmedEmail, password, options: { emailRedirectTo: window.location.origin } });
        if (error) { toast.error(error.message); }
        else if (data.user?.identities?.length === 0) { toast.error(t("auth.emailAlreadyExists")); }
        else if (data.user && !data.session) { toast.success(t("auth.confirmEmail")); }
      }
    } catch { toast.error(t("auth.unexpectedError")); }
        ...
        <div className="glass-card rounded-3xl gradient-border p-6 sm:p-8">
          <div className="text-center space-y-2 mb-6">
            <h1 className="text-2xl sm:text-3xl font-black text-foreground tracking-tight">
              {isForgotPassword ? t("auth.forgotPassword") : isLogin ? t("auth.welcome") : t("auth.createAccount")}
            </h1>
            ...
            <div className="space-y-4">
              <form onSubmit={handleForgotPassword} className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="email" className="text-foreground text-sm font-semibold">{t("auth.email")}</Label>
                  <div className="relative group">
                    ...
                    <Input id="email" type="email" placeholder={t("auth.emailPlaceholder")} value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10 h-12 rounded-xl bg-background/50 border-border/50 focus:border-primary/50 focus:ring-2 focus:ring-primary/20 transition-all" required />
                  </div>
                </div>
                <Button type="submit" className="w-full h-12 rounded-xl font-bold text-base glow-primary-sm hover:glow-primary transition-all" disabled={loading}>
                  {loading ? t("common.sending") : t("auth.sendRecoveryLink")}
                </Button>
              </form>
              <p className="text-center text-sm text-muted-foreground">
                <button onClick={() => setIsForgotPassword(false)} className="text-primary font-semibold underline-offset-4 hover:underline">{t("auth.backToLogin")}</button>
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label htmlFor="password" className="text-foreground text-sm font-semibold">{t("auth.password")}</Label>
                    {isLogin && (
                      <button type="button" onClick={() => setIsForgotPassword(true)} className="text-xs text-primary font-semibold underline-offset-4 hover:underline">{t("auth.forgotPasswordLink")}</button>
                    )}
                  </div>
                  <div className="relative group">
              ...
              <div className="relative">
                ...
                <div className="relative flex justify-center text-xs"><span className="bg-card/80 px-3 text-muted-foreground">{t("common.or")}</span></div>
              </div>

              <p className="text-center text-sm text-muted-foreground">
                {isLogin ? t("auth.noAccount") : t("auth.hasAccount")}{" "}
                <button onClick={() => setIsLogin(!isLogin)} className="text-primary font-semibold underline-offset-4 hover:underline">{isLogin ? t("auth.signup") : t("auth.login")}</button>
              </p>
            </div>
          )}
        </div>
